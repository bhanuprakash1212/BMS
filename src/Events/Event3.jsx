...
const Event3Page = () => {
    ...
    if (participationCount < participationLimit) {
      try {
        ...
        await updateDoc(docRef, {
          participatedEvents: arrayUnion("Event 3"),
          participationCount: parseInt(participationCount) + 1,
        });
        fetch();
        setPop(false);
        setAlert({ open: true, message: "Added to Event 3", severity: "success" });
      } catch (error) {
  ...
  const handleRemoveFromEvent = async (id, participationCount) => {
    if (participationCount > 0) {
      try {
        const docRef = doc(db, "Applicant", id);
        await updateDoc(docRef, {
          participatedEvents: arrayRemove("Event 3"),
          participationCount: parseInt(participationCount) - 1,
        });
        fetch();
        setAlert({ open: true, message: "Removed from Event 3", severity: "success" });
      } catch (error) {
  ...
  const filteredData = fetchd.filter((person) =>
    person.participatedEvents?.includes("Event 3") &&
  ...
  return (
    <div className="event3-page">
      <Dialog open={pop} onClose={() => setPop(false)} maxWidth="md" fullWidth>
        <DialogTitle>Add Applicant to Event 3</DialogTitle>
        <DialogContent>
          <TextField
            fullWidth
            variant="outlined"
