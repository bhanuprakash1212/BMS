...
const Event2Page = () => {
    ...
    if (participationCount < participationLimit) {
      try {
        ...
        await updateDoc(docRef, {
          participatedEvents: arrayUnion("Event 2"),
          participationCount:parseInt(participationCount) + 1,
        });
        fetch();
        setPop(false);
        setAlert({ open: true, message: "Added to Event 2", severity: "success" });
      } catch (error) {
  ...
  const handleRemoveFromEvent = async (id, participationCount) => {
    if (participationCount > 0) {
      try {
        const docRef = doc(db, "Applicant", id);
        await updateDoc(docRef, {
          participatedEvents: arrayRemove("Event 2"),
          participationCount: parseInt(participationCount) - 1,
        });
        fetch();
        setAlert({ open: true, message: "Removed from Event 2", severity: "success" });
      } catch (error) {
  ...
  const filteredData = fetchd.filter((person) =>
    person.participatedEvents?.includes("Event 2") &&
  ...
  return (
    <div className="event2-page">
      <Dialog open={pop} onClose={() => setPop(false)} maxWidth="md" fullWidth>
        <DialogTitle>Add Applicant to Event 2</DialogTitle>
        <DialogContent>
          <TextField
            fullWidth
            variant="outlined"
